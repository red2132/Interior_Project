...
public interface QnAMapper {
	...
	*/
	...
	//1. Q&A 게시글 리스트
		...
		//1.댓글 업로드
		@SelectKey(keyProperty="no", resultType=int.class, before=true,
			     statement="SELECT NVL(MAX(no)+1,1) as no FROM qna_reply")
		@Insert("INSERT INTO qna_reply(no,bno,id,msg,g_id) "
				+ "VALUES(#{no},#{bno},#{id},#{msg},(SELECT NVL(MAX(g_id)+1,1) FROM qna_reply))")
		...
		public void qnaBoareReplyReplyInsert(QnaReplyVO vo);
		//4-4. depth 증가
		@Update("UPDATE qna_reply SET "
				+ "depth=depth+1 "
				+ "WHERE no=#{no}")
		...
		public void qnaBoardReplyMsgUpdate(int no);
		//4-2. depth-1 감소

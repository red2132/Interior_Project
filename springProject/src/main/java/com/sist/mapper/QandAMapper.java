...
public interface QandAMapper {
	...
	*/
	...
	@Select("SELECT no,subject,id,regdate,hit,g_tab,num "
			...
			+ "FROM project_qna ORDER BY g_id DESC,g_step ASC)) "
			+ "WHERE num BETWEEN #{start} AND #{end}")
	public List<QandAVO> qnaBoardListData(Map map);
	//1-2. 총페이지 설정
	@Select("SELECT CEIL(COUNT(*)/10.0) FROM project_qna")
	...
	@SelectKey(keyProperty="no", resultType=int.class,before=true,
			statement="SELECT NVL(MAX(no)+1,1) as no FROM project_qna")
	@Insert("INSERT INTO project_qna(no,id,subject,content,pwd,g_id) VALUES("
			+ "#{no},#{id},#{subject},#{content},#{pwd},(SELECT NVL(MAX(g_id)+1,1 FROM project_qna))")
	public void qnaBoardInsert(QandAVO vo);
	
	
	//3. Q&A 게시글/답변게시글 상세보기
	//3-1. 게시글 클릭 시 hit 증가
	@Update("UPDATE project_qna SET hit=hit+1 WHERE no=#{no}")
	...
	@Select("SELECT g_id,g_step,g_tab FROM project_qna WHERE no=#{no}")
	public QandAVO qnaParentInfoData(int no);
	//4-2. 답변 시 답변게시글의 group_step +1 증가
	@Select("UPDATE project_qna SET "
			+ "g_step=g_step+1 "
	...
	@SelectKey(keyProperty="no",resultType=int.class,before=true,
			statement="SELECT NVL(MAX(no)+1,1) as no FROM project_qna")
	@Insert("INSERT INTO project_qna(no,id,subject,content,pwd,g_id,g_step,g_tab,root,depth) VALUES("
			+ "#{no},#{id},#{subject},#{content},#{pwd},#{g_id},#{g_step},#{g_tab},#{root},0)")
	public void qnaBoardReplyInsert(QandAVO vo);
	//4-4. depth 증가
	@Update("UPDATE project_qna SET depth=depth+1 WHERE no=#{no}")
	...
	//5. Q&A 게시글/답변게시글 수정
	//5-1. 비밀번호 검색
	@Select("SELECT pwd FROM project_qna WHERE no=#{no}")
	public String qnaBoardGetPassword(int no);
	//5-2. 실제 게시글 수정
	@Update("UPDATE project_qna SET subject=#{subject},content=#{content} "
			+ "WHERE no=#{no}")
	...
	@Select("SELECT root,depth FROM project_qna WHERE no=#{no}")
	public QandAVO qnaBoardGetDepth(int no);
	//6-2-2. depth==0(답변게시글이 없을 때): 삭제
	@Delete("DELETE FROM project_qna WHERE no=#{no}")
	public void qnaBoardDelete(int no);
	//6-2-3-1. depth!=0(답변게시글이 있을 때): 
	@Update("UPDATE project_qna SET subject=#{subject},content#{content} WHERE no=#{no}")
	public void qnaBoardDeleteUpdate(QandAVO vo);
	//6-2-3-2. depth 감소
	@Update("UPDATE project_qna SET depth=depth-1 WHERE no=#{no}")
	public void depthDecrement(int no);
	//6-2-4. depth 읽어오기
	@Select("SELECT depth FROM project_qna WHERE no=#{no}")
